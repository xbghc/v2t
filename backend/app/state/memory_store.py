...
from app.storage import cleanup_old_files, get_file_storage, get_metadata_store
...
def set_status_queue(workspace_id: str, queue) -> None:
    """注册 SSE 状态推送队列"""
    _status_queues[workspace_id] = queue


def get_status_queue(workspace_id: str):
    """获取 SSE 状态推送队列"""
    return _status_queues.get(workspace_id)
...
async def cleanup_old_workspaces() -> None:
    ...
    - 本地存储中的资源文件：按工作区目录批量删除
    - MongoDB 中的 workspace 元数据：基于 last_accessed_at 清理
    - 本地临时文件目录：基于目录 atime 清理
    """
    store = get_metadata_store()
    ...
    for ws in expired:
        # 删除本地存储中的资源文件（按工作区目录）
        await file_storage.delete_prefix(ws.workspace_id)
